...
# dataset_path = os.getcwd() + os.sep + 'dataset' + os.sep
cur_root = os.getcwd()
parent_root = os.path.dirname(cur_root)
dataset_path = parent_root + os.sep + 'dataset' + os.sep
source_data_path = dataset_path + 'source_data' + os.sep
save_path = dataset_path + 'autolabel_result_data' + os.sep
dict_path = dataset_path + "medical_ner_dict.csv"  # 医疗实体词典：实体、实体类别
mode = "max_seg"  # 实体词典+双向最大匹配。mode="jieba" 实体词典+jieba词性标注

""" 医疗实体词典：实体、实体类别, 每一行类似：(视力减退,SYM) """
...
def auto_label(files, data_type, mode="jieba"):
    ...
    是实体，则标记为BI。
    """
    writer = open(save_path+"%s_data.txt" % data_type,"w",encoding="utf8")
    for file in files:
        fp = open(source_data_path+file,'r',encoding='utf8')
        for line in fp:
            """ 按词性分词 """
...
def main():
    ...
    for files, data_type in zip([trains,devs,tests],["train","dev","test"]):
        auto_label(files, data_type, mode)
